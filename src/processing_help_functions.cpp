/**
 * @file processing_help_functions.cpp
 * @author Adam Gabrys
 * @login xgabry01
 */
...
int ldap_functions::get_mess_length() 
{
    ...
    // Add the final byte (low 7 bits) to the length with or operation
    length |= (byte_content << shift);

    return length ;
}

void ldap_functions::get_base_content(int base_content_legth) 
{
    dn = "";
    if(base_content_legth == 0) return; //

    next_byte(client_message_header, 1); //move from the byte with dn length content
    for (int i = 0; i < base_content_legth - 1 ; i++) 
    {
        dn += byte_content;
        next_byte(client_message_header, 1);
...
/// @param s 
/// @return TLV format of string in hex
string ldap_functions::LV_string(string s) 
{
    ...
    else 
    {
        ...
        while (length > 0) 
        {
            ...
            length = length >> 7;   //shift the bits to the right by 7    
